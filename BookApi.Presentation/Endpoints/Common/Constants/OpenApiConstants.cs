namespace Library.Presentation.Endpoints.Common.Constants;

internal static class OpenApiConstants
{
    public const string BookPrimaryGroup = nameof(Book);
    public const string LoginPrimaryGroup = nameof(Login);
    public const string DefaultPrimaryGroup = "DefaultPrimaryGroup";
    public const string QueriesSubGroup = "Queries";
    public const string CommandsSubGroup = "Commands";
}